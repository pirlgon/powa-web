...
class DatabaseOverview(DashboardPage):
    ...
    datasources = [DatabaseOverviewMetricGroup, ByQueryMetricGroup,
                   ...
                   ByFuncUserFuncMetricGroup, DatabasePGSAOverview,
    ...
    def dashboard(self):
        ...
        pgsa_metrics = DatabasePGSAOverview.split(self,
                                                  [["backend_xid_age",
                                                    "backend_xmin_age",
                                                    "oldest_backend",
                                                    "oldest_xact",
                                                     "oldest_query"],
                                                    parallel_metrics])
        db_graphs.append([Graph("Global activity (On database %(database)s)",
                                metrics=pgsa_metrics[0],
                                renderer="bar",
                                stack=True)])
        if (len(pgsa_metrics[2]) > 0):
            db_graphs[1].append(Graph("Parallel query (On database %(database)s)",
                                      metrics=pgsa_metrics[2]))
        db_graphs[1].append(Graph("Backend age (On database %(database)s)",
        ...
        global_db_graphs = [[Graph("Transactions per second",
                             metrics=[DatabaseDbActivityMetricGroup.xact_commit,
                                      DatabaseDbActivityMetricGroup.xact_rollback],
                             renderer="bar",
                             stack=True),
                             Graph("Conflicts & deadlocks",
                                   metrics=[DatabaseDbActivityMetricGroup.conflicts,
        ...
        all_rel_graphs = [[Graph("Access pattern",
                          ...
                          [Graph("Vacuum activity",
                           ...
                           Graph("Object size",
                           metrics=[DatabaseAllRelMetricGroup.tbl_size,
